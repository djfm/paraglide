...
const addInfo = tagToApply => tree => {
  if (isTree(tree)) {
    if (tagToApply === true && tree.recognized) {
      return tree;
    }
    if (tagToApply && (tree.recognized === true || !tree.recognized)) {
      return {
...
const makeRecognizedTree = tagToApply =>
  flatMapArgs(
    (...nodes) => {
      const taggedNodes = nodes.map(addInfo(tagToApply));
      const grouped = groupByRecognized(taggedNodes);
